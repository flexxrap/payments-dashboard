...
- **Статусы актов:** `not_sent / waiting_signature / closed / attention_required` — расчёт на
  backend, кнопки «отправлен»/«подписан» с возможностью отката («отменить отправку»/«снять
  подпись») для исправления ошибочного клика, правка комментария с сохранением. Снять отправку
  можно только сняв подпись (нельзя «подписан, но не отправлен» — инвариант держится на backend).
