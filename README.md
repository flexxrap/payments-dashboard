...
- **Парсинг банковских выписок.** Загрузка PDF-выписки → разбор через `pdfplumber` → матчинг
  строк выписки с оплатами (по сумме, дате, номеру счёта/договора) → автосоздание `Payment`
  и `Act`. Это убирает ручной ввод оплат.
